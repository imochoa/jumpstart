...
def cogpre_header(d: dict[str, str]) -> str:
    """
    cog "prologue" applied to any ".pre" cog segments in files
    """
    return f'cog.outl("# <<<PRE STAGE>>>")\n' f'cog.outl("")\n'


def cogmain_header(d: dict[str, str]) -> str:
    """
    cog "prologue" applied to any cog segments in files
    """
    name = d.get("NAME", "")
    template = d.get("TEMPLATE", "")
    file = d.get("FILE", "")
    return (
        f'cog.outl("# <<<AUTOGENERATED - DO NOT MODIFY>>>")\n'
        f'cog.outl("#\t\t > {file}")\n'
        f'cog.outl("#\t\t > {name}")\n'
        f'cog.outl("#\t\t > {template}")\n'
        f'cog.outl("")\n'
...
def cogpost_header(d: dict[str, str]) -> str:
    """
    cog "prologue" applied to any ".post" cog segments in files
    """
    return f'cog.outl("# <<<POST STAGE>>>")\n' f'cog.outl("")\n'
...
def cog_param(
    ...
    for src, dst in template_map.items():
        ...
        p = cog_subprocess(
            ...
            cog_args=cog_args,
            prologue=cogmain_header(cog_args),
        )
        if p.returncode != 0:
    ...
    if post_dir.is_dir():
        ...
        for src, dst in post_template_map.items():
            ...
            post_p = cog_subprocess(
                ...
                prologue=cogpost_header(post_cog_args),
            )
