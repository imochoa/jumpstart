...
DLTMP=$(mktemp -d -t jumpstart-XXXXXXXXXX) \
    ...
    && URL=$(curl --silent 'https://api.github.com/repos/fiatjaf/jiq/releases/latest' | jq '..|.browser_download_url?' | grep 'x86_64' | grep 'linux' | grep 'gnu' | tr -d '"') \
    && curl -jLO "${URL}" \
    && DLFILE=$(ls . | head -n1) \
    && printf "\e[0;34m%-6s\e[m\n" "Extracting..." \
    && tar -xzvf "${DLFILE}" \
